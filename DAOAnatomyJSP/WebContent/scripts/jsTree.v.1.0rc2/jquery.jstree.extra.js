...
(function ($) {
	$.jstree.plugin("extra", {
		...
		__destroy : function () {
			this.get_container().find(".jstree-extra").remove();
		},
		_fn : {
			_prepare_checkboxes : function (obj) {
				...
				obj.each(function () {
					...
					t.find("a").not(":has(.jstree-extra)").after("<ins class='jstree-extra'>&#160;</ins>").parent().not(".jstree-checked, .jstree-unchecked").addClass(c);
				});
