...
public class CheckComponents {
    ...
    private ArrayList<Component> proposedTermList = null;

    //reference OBO file
    private ArrayList<Component> referenceTermList = null;

    //components that have passed red check
    private ArrayList<Component> passRedTermList = null;

    //components that have passed blue check
    private ArrayList<Component> passBlueTermList = null;

    //components (new/existing) with proposed changes => combobox ChangedNodes
    private ArrayList<Component> changesTermList = null;

    //components that have not passed red / blue check => combobox ProblemNodes
    private ArrayList<Component> problemTermList = null;

    //components that are group terms ..delete after testing
    private ArrayList<Component> groupTermList = null;

    //components that are abstract anatomy terms excluding roots =>
    // used for ontology rules check
    private ArrayList<Component> abstractTermList = null;
    ...
    public CheckComponents(ArrayList<Component> newTermList,
            ArrayList<Component> oldTermList,
        ...
        // children of new groups with calculated common ancestor will not have
        // any rule violations anymore
        ...
        this.validateConfiguredRoots(treebuilder, abstractClass, stageClass,
                groupClass, groupTermClass);
        this.abstractTermList = this.getAbstractAnatomyChildren(treebuilder,
                abstractClass, groupClass);
        ...
        System.out.println("CheckComponents Class: checking rules and " +
                "changes completed.");
    }
    ...
    //does not include abstract terms that are tree roots (eg. mouse, human
    // or abstract terms whose parent link has been deleted)
    private ArrayList getAbstractAnatomyChildren(TreeBuilder treebuilder, 
            Component abstractClass,
            Component groupClass){
        
        ArrayList<Component> abstractAnatomyChildren =
        ...
        if ( abstractRootList.isEmpty() ){
            ...
            for (int i = 0; i < this.proposedTermList.size(); i++){
                ...
                if ( compie.getNamespace().equals( 
                        abstractClass.getNamespace() ) ) {
                    abstractAnatomyChildren.add( compie );
                }
                //is a new group term
                if ( compie.getNamespace().equals(
                        groupClass.getNamespace() ) &&
                        !vRoots.contains( compie.getID() ) ) {
                    abstractAnatomyChildren.add( compie );
                }
            }
            
            //for (Component compie: abstractAnatomyChildren ){
            ...
            for (int k = 0; k < this.proposedTermList.size(); k++){
                ...
                for (int j = 0; j < this.abstractRootList.size(); j++ ){
                    ...
                    //REPLACE isSamsAs with matching id, two compies can be
                    // the same but have modified properties
                ...
                if ( compie.commentsContain("INFO: Obsolete Term") )
                    System.out.println("Obsolete Term Detected: " +
                      compie.getID());
                    abstractAnatomyChildren.remove( compie );
                */
            }
    ...
    private void validateConfiguredRoots(TreeBuilder treebuilder, 
        ...
        for(String emapID: roots){
            ...
            /*
            System.out.println("--------------------");
            System.out.println("abstractID         = " + abstractID );
            System.out.println("emapID             = " + emapID );
            System.out.println("abstractNameSpace  = " + abstractNameSpace );
            ...
            System.out.println("stageNameSpace     = " + stageNameSpace);
            System.out.println("groupNameSpace     = " + groupNameSpace);
            System.out.println("groupTermNameSpace = " + groupTermNameSpace);
            System.out.println("stageName          = " + stageName);
            System.out.println("groupName          = " + groupName);
            System.out.println("groupTermName      = " + groupTermName);
            System.out.println("rootNameSpace      = " + rootNameSpace);
            System.out.println("--------------------");
            */

            if ( abstractID.equals( emapID ) && 
                 abstractNameSpace.equals( rootNameSpace ) &&
                 abstractName.equals( rootName ) ) {
                //rootCompie.setStrRuleStatus("PASSED");
                //note not the tree component - but gui ref component
                abstractClass.setStrRuleStatus("PASSED"); 
                //set abstract anatomy tree roots
                this.abstractRootList.add( rootCompie );
            }
            else if ( stageID.equals( emapID ) && 
                      stageNameSpace.equals( rootNameSpace ) &&
                      stageName.equals( rootName ) ) {
                stageClass.setStrRuleStatus("PASSED");
                //set root group_term to isPrimary = false to exclude primary
                // paths leading back to this term from pool of possible
                // primary paths
                rootCompie.setIsPrimary(false);
            }
            else if ( groupID.equals( emapID ) && 
                      groupNameSpace.equals( rootNameSpace ) &&
                      groupName.equals( rootName ) ) {
            ...
            else if ( groupTermID.equals( emapID ) && 
                      groupTermNameSpace.equals( rootNameSpace ) &&
                      groupTermName.equals( rootName ) ) {
                ...
                //don't allow to fail
                rootCompie.setStrRuleStatus("PASSED");
                rootCompie.setCheckComment("Component has been deleted " +
                ...
                rootCompie.setCheckComment("INFO: Root node not defined by " +
                        "OBO2DB NameSpace Configurations.");
                //make sure it isn't added twice
                this.problemTermList.remove(rootCompie); 
                this.problemTermList.add(rootCompie);
            }
        }
    }
    
    //path checking performed for all components (in abstract anatomy except
    // mouse)
    ...
    private void validatePaths(TreeBuilder treebuilder,
        ...
        for ( Component compie: this.abstractTermList ){
            ...
            //reset the primary path for all components to null everytime this
            // method is called
            ...
            for (DefaultMutableTreeNode[] path: paths){
                ...
                if ( isPrimaryPath ){
                    if ( compie.getPrimaryPath() == null ) {
                        ...
                        TreePath printPath = 
                                new TreePath( compie.shortenPath( path) );
                        compie.setCheckComment("More than one primary path! " +
                    ...
                    if ( treebuilder.isPathInNamespace(path, abstractClass) ) {
                        compie.addPaths(path);
                    }
                }
            }
        }
    ...
    private void checkAbstractAnatomyParents(TreeBuilder tree){
        ...
        for ( Component compie: this.abstractTermList ){
            ...
            if ( primaryParents > 1 ) {
                ...
                compie.setCheckComment("More than one primary parent! " +
    ...
    private void checkAbstractAnatomyLinks() {
        ...
        for (int i = 0; i < this.abstractTermList.size(); i++){
            compie = this.abstractTermList.get(i);
                ...
                if (compie.getEndsAt() == -1 ) {
                    /*System.out.println(compie.getID() + ": " +
                     "Relation: Ends At -- Missing ends_at stage!");*/
                    compie.setFlagMissingRel(true);
                    compie.setCheckComment("Relation: ends_at -- " +
                    ...
                    this.problemTermList.add(compie);
                    compie.setStrRuleStatus("FAILED");
                    //compie.setEndsAt("TS01");
                }
                //check missing starts_at stage
                if (compie.getStartsAt() == -1 ) {
                    /*System.out.println(compie.getID() + ": " +
                      "Relation: Starts At -- Missing starts_at stage!");*/
                    compie.setFlagMissingRel(true);
                    compie.setCheckComment("Relation: starts_at -- Missing " +
                      "starts at stage - Component's stage range cannot be " +
                      "determined.");

                    compie.setStrRuleStatus("FAILED");
                    //compie.setStartsAt("TS01");
                }
                //check missing part_of link
                if (compie.getIsPrimary() ) {
                    //if (compie.getPartOf().isEmpty() &&
                    //        compie.getIsA().isEmpty() ){
                    if (compie.getPartOf().isEmpty() ){
                        /*System.out.println(compie.getID() + ": " +
                         "Relation: Part Of -- No parent entry!");*/
                        compie.setFlagMissingRel(true);
                        compie.setCheckComment("Relation: part_of -- " +
                                "Missing relationship - Component has no " +
                                "parents.");
                        compie.setStrRuleStatus("FAILED");
                    }
                    /*
                    if (!compie.getPartOf().isEmpty() &&
                        ...
                        System.out.println(compie.getID() + ": " +
                          "Relation: Part Of -- No parent entry!");
                        compie.setFlagMissingRel(true);
                        compie.setCheckComment("Relation: part_of AND " +
                          "is_a: -- Both Present!");
                        compie.setStrRuleStatus("FAILED");
                    }
                    */
                }
                //if group component and missing group part of link
                //must cross check with ref file
                //cannot allow user to create own groups
                if ( !compie.getIsPrimary() &&
                      compie.getGroupPartOf().isEmpty() ){
                    compie.setFlagMissingRel(true);
                    compie.setCheckComment("Relation: group_part_of " +
                            "-- Missing relationship - Group component has " +
                            "no parents.");
                    compie.setStrRuleStatus("FAILED");
                }
                ...
                if ( compie.getStartsAt() != -1 &&
                     compie.getEndsAt() != -1 &&
                     compie.getEndsAt() < compie.getStartsAt() ) {
                    /*System.out.println(compie.getID() + ": " +
                     "Relation: Ends At + Starts At -- Ends_at stage " +
                     "earlier than Starts_at stage!");*/
                    compie.setFlagMissingRel(true);
                    compie.setCheckComment("Relation: starts_at, ends_at " +
                ...
                if (this.species.equals("mouse")) {
                    if ( compie.getStartsAt() != -1 &&
                         compie.getEndsAt() != -1 &&
                        (compie.getStartsAt() < 0) ||
                        (compie.getEndsAt() > 27) ){
                    /*System.out.println(compie.getID() + ": " +
                     "Relation: Stages are out of range! [Start: " +
                     Integer.toString(compie.getStartsAt()) + ", Ends: " +
                ...
                //if ( compie.getCheckComments().contains(
                ...
                if ( compie.commentsContain("Broken Link: Phantom parent") ){
                    /*System.out.println(compie.getID() + ": " +
                     ...
                     "file!");*/
                    compie.setFlagMissingRel(true);
                    compie.setCheckComment("Orphan component -- Phantom " +
    ...
    //for the primary path, each node must be within the lifetime of the
    // preceding node
    private void checkAbstractAnatomyStages(){
        ...
        for (Component compie: this.passRedTermList) {
            ...
            for(int pointer=2; pointer < primaryPath.length - 1 && proceed;
                pointer++){
                Component parent =
                        (Component) primaryPath[pointer].getUserObject();
                Component child =
                        (Component) primaryPath[pointer+1].getUserObject();

                boolean within = ( parent.getStartsAt() != -1 &&
                                   child.getStartsAt() != -1 &&
                                   parent.getEndsAt() != -1 &&
                                   child.getEndsAt() != -1 &&
                        ( parent.getStartsAt() <= child.getStartsAt() ) &&
                        ( parent.getEndsAt() >= child.getEndsAt() ) );
                if ( compie.getID().equals("EMAPA:31495") && !within ){
                    /*System.out.println( "Checking stage range for " +
                     child.getID() );
                    ...
                    System.out.println( "parent " + parent.getID() +
                     " starts at Sequence" + parent.getStartsAt() );
                    System.out.println( "parent " + parent.getID() +
                     " ends at Sequence" + parent.getEndsAt() );
                    System.out.println( "parent.getStartsAt() <= " +
                     "child.getStartsAt = " +
                     Boolean.toString(parent.getStartsAt() <=
                     child.getStartsAt()) );
                    System.out.println( "parent.getEndsAt() >= " +
                     "child.getEndsAt = " + parent.getEndsAt() +
                     ">=" + child.getEndsAt() + " " +
                     Boolean.toString(parent.getEndsAt() >=
                     child.getEndsAt()) );*/
                }

                //boolean default_proceed = (
                // !parent.getNamespace().equals("abstract_anatomy") );
                if (!within) {
                    compie.setFlagLifeTime(true);
                    compie.setStrRuleStatus("FAILED");
                    compie.setCheckComment("Stage out of range: [" +
                            child.getID() + " " +
                            child.getName() + " " + 
                            child.getStartsAtStr(this.species) + "-" +
                            child.getEndsAtStr(this.species) +
                            "] is not within lifetime of parent [" + 
                            parent.getID() + " " +
                            parent.getName() + " " +
                            parent.getStartsAtStr(this.species) + "-" +
                            parent.getEndsAtStr(this.species) + "]");
                    this.problemTermList.add(compie);
                }
            }
        }
    ...
    private void checkOrdering(TreeBuilder tree,
        ...
        for (Enumeration<DefaultMutableTreeNode> eRootChildren = 
            ...
            Component rootChildCompie =
                    (Component) eRootChildren.nextElement().getUserObject();
        ...
        for (DefaultMutableTreeNode nodie: allNodes){
            ...
            proceed = true;
            //get all children
            for (Enumeration<DefaultMutableTreeNode> eChildren = 
                    nodie.children() ;
                    eChildren.hasMoreElements() ;) {
                childrenCompie.add(
                        (Component) eChildren.nextElement().getUserObject() );
            }
            ...
            proceed = parentCompie.getStrRuleStatus().equals("FAILED") ?
                false : true;
            //stop order checking if any of the children fail
            for (Component compie: childrenCompie){
                failedChild = compie.getStrRuleStatus().equals("FAILED") ?
                    true : false ;
                if (failedChild &&
                        !parentCompie.getStrRuleStatus().equals("FAILED") ){
                    //set fail to parent
                    parentCompie.setCheckComment("Ordering: The ordering " +
            ...
            for (int k=0; k<childrenCompie.size() && proceed; k++){
                ...
                String[] arrOrderComments = 
                        childCompie.getOrderCommentOnParent(
                        parentCompie.getID());
            ...
            if ( !childrenOrder.isEmpty() &&
                ...
                //set fail to parent
                parentCompie.setCheckComment("Ordering: One of this "+
                        "component's children has a duplicate order sequence.");
                parentCompie.setFlagMissingRel(true);
                parentCompie.setStrRuleStatus("FAILED");
                this.problemTermList.add(parentCompie);
                for (Component compie: childrenCompie){
                    compie.setCheckComment("Ordering: One of the siblings " +
            ...
            // order
                ...
                for (int i=0; i<=intMaxOrder && !flagStop; i++){
                    ...
                    for (int k=0; k<childrenOrder.size() && notMatch; k++){
                        ...
                        if ( strOrder.equals(Integer.toString(i)) ){
                            /*System.out.println("Child of " +
                             parentCompie.getID() + " has order " + strOrder);*/
                            notMatch=false;
                        } 
                    }
                    ...
                    if ( notMatch ){
                        ...
                        //set fail to parent
                        parentCompie.setCheckComment("Ordering: One of " +
                        ...
                        for (Component compie: childrenCompie){
                            compie.setCheckComment("Ordering: One of the " +
    ...
    private void checkChanges( Component abstractClass ) {
        ...
        for (Iterator<Component> i = this.abstractTermList.iterator();
             i.hasNext();){
        //for (Iterator<Component> i = this.proposedTermList.iterator();
        //     i.hasNext();) {
            proposed = i.next();
            ...
            for (Iterator<Component> k = this.referenceTermList.iterator();
                 k.hasNext() && !flagFound;) {
                reference = k.next();

                /*System.out.println("Component: " + proposed.getID() + " " +
                        proposed.getStrChangeStatus() + " " +
                        proposed.getCheckComments() );*/
                ...
                if ( proposed.getID().equals( reference.getID() ) ) {
                    ...
                    if (proposed.isSameAs(reference) &&
                        ...
                        //set to unchanged 
                        /*System.out.println("Unchanged component detected: " +
                         proposed.getID());*/
                        proposed.setStrChangeStatus("UNCHANGED");
                    }
                    //else mark green in newTermList and add to new ArrayList
                    //can't detect deleted components here because component
                    // has been removed from abstractTermList
                    //else if ( proposed.getStrChangeStatus().equals("DELETED")
                    // && proposed.commentsContain("INFO: Obsolete Term")){

                    if ( proposed.isSameAs(reference) &&
                            proposed.commentsContain("INFO: Obsolete Term")){
                        /*System.out.println("Deleted component detected: " +
                         proposed.getID());*/
                        proposed.setStrChangeStatus("DELETED");
                        ArrayList<String> formerParents = reference.getPartOf();
                        formerParents.addAll( reference.getGroupPartOf() );
                        proposed.setCheckComment("Obsolete term formerly " +
                                "linked to : " + formerParents);
                        //<=== this is the component arraylist to generate
                        //     SQL queries!!!
                    ...
                    if (!proposed.isSameAs(reference) &&
                            !proposed.commentsContain("INFO: Obsolete Term")) {
                        /*System.out.println("Changed component detected: " +
                         proposed.getID());*/
                        proposed.setStrChangeStatus("CHANGED");
                        /*System.out.println("Difference detected, " +
                         "compared proposed: " + proposed.getID() +
                         " vs reference: " + reference.getID());*/
                        ArrayList<String> arrDifference =
                                proposed.getDifferenceWith(reference, this.species);
            ...
            //if not found, 
            if ( !flagFound &&
                    proposed.getStrChangeStatus().equals("DELETED") ) {
                proposed.setCheckComment("Obsolete term does not exist in " +
                        "database. No deletion will take place.");
                this.changesTermList.add(proposed);
            }
            else if ( !flagFound ){
        ...
        for (Iterator<Component> i = this.referenceTermList.iterator();
             i.hasNext();) {
            reference = i.next();
            
                flagFound = false;
                //look for component in newTermList
                for (Iterator<Component> k = this.proposedTermList.iterator();
                     k.hasNext() && !flagFound;) {
                    proposed = k.next();
                ...
                //if not found
                if (!flagFound && 
                        reference.getNamespace().equals(
                            abstractClass.getNamespace() ) ) {
                    reference.setStrChangeStatus("DELETED");
                    reference.setStrRuleStatus("FAILED");   
                    reference.setCheckComment("INFO: Component was deleted " +
                            "from OBO file bypassing obsolete procedures.");
                    reference.setFlagMissingRel(true);

                    //add comment for parents in reference file
                    ArrayList<String> formerParents = reference.getPartOf();
                    formerParents.addAll( reference.getGroupPartOf() );
                    reference.setCheckComment("Obsolete term used to have " +
                            "former parents: " + formerParents);

                    //remove parents and add back to proposedlist to display as
                    // root
    ...
    //not necessarily the same treebuilder as the one used and instantiated in
    // the constructor
    public Component getCommonAncestor(DefaultMutableTreeNode groupNode,
            TreeBuilder treebuilder){
        ...
        Vector< DefaultMutableTreeNode > childNodes =
                new Vector< DefaultMutableTreeNode >();
        childNodes = treebuilder.recursiveGetNodes( groupNode, childNodes );
        
        //get primary paths for all child nodes
        Vector< DefaultMutableTreeNode[] > primaryPaths =
                new Vector< DefaultMutableTreeNode[] >();
        for(int i = 0; i < childNodes.size(); i++){
            descCompie = (Component) childNodes.get(i).getUserObject(); 
            //if one descendant does not have a primary path, common ancestor
            // cannot be determined
            if ( descCompie.getPrimaryPath() == null ) {
                return compie;
            }
            primaryPaths.add( descCompie.getPrimaryPath() ) ;
            //get the earliest start stage and latest end stage amongst all
            // descendants
            /*earliestStart = ( descCompie.getStartsAt() < earliestStart ) ?
                descCompie.getStartsAt() : earliestStart;
            latestEnd = ( descCompie.getEndsAt() > latestEnd ) ?
                descCompie.getEndsAt() : latestEnd;*/
            if (this.species.equals("mouse")){
                 if ( descCompie.getStartsAt() != -1 &&
                      descCompie.getStartsAt() < 27) {
                    earliestStart = descCompie.getStartsAt();
                }
                else {
                    earliestStart = 27;
                }
                if ( descCompie.getEndsAt() != -1 &&
                     descCompie.getEndsAt() > 0) {
                    latestEnd = descCompie.getEndsAt();
                }
                else {
                    latestEnd = 0;
                }
        ...
        //if there are primary paths find the common ancestor for all primary
        // paths
        if ( !primaryPaths.isEmpty() ){   
            ...
            if (o instanceof Component){
                ...
                //create a clone of the common ancestor to use for passing the
                // calculated lifetime for the new group component
                copyCompie = compie.clone();
                copyCompie.setStartsAtInt( earliestStart );
                copyCompie.setEndsAtInt( latestEnd );
            }
        }
        return copyCompie;
    }
    ...
    public void clearStatusComments( ArrayList < Component > termList ){
        ...
        for ( Component compie: termList ){
            //clear everything except deleted terms that can't be detected any other way
            // and recently added to common ancestor group components
            if ( !compie.commentsContain("INFO: Obsolete Term") && 
                 !compie.commentsContain("New Group Component") ) { 
    ...
    public ArrayList getNewTermList(){
        ...
        for ( Component term: this.changesTermList ){
            if ( term.getStrChangeStatus().equals("NEW") ) {
                newTerms.add( term );
            }
        }
    ...
    public ArrayList getDeletedTermList(){
        ...
        for ( Component term: this.changesTermList ){
            if ( term.getStrChangeStatus().equals("DELETED") ) {
    ...
    public ArrayList getModifiedTermList(){
        ...
        for ( Component term: this.changesTermList ){
            if ( term.getStrChangeStatus().equals("CHANGED") ) {
