...
public class TreeBuilder {
    ...
    public TreeBuilder(MapBuilder mappie, String species){
        ...
        for(int i=0; i< vRoots.size(); i++){
            //Component compie =
            // (Component) this.mapProperties.get(vRoots.get(i));
    ...
    public DefaultMutableTreeNode recursiveAddNode(String key,
        ...
        if ( compie == null ) {
            ...
            compie.setCheckComment("Disallowed deletion of component from " +
        ...
        if ( this.containsComponent(parent_path, compie) ) {
            ...
            compie.setCheckComment("There appears to be a cycle in your " +
                    "graph (" + compie.getID() + " appears more than once " +
                    "in " + printPath + ")");
            return newnode;
        } 
        ...
        //create new path for each component, copy old path passed from
        // last traversal
        ...
        //add current component to new path; add new path to paths for
        // current component
        ...
        //traverse down each branch of the tree to add all child nodes and
        // create paths
        if ( v != null ) {
    ...
    public void writeNodeReport( String strFileName, String mothernodeID ){
        try{
            ...
            DefaultMutableTreeNode dummy = 
                    recursiveWriteNode( mothernodeID,
                    new DefaultMutableTreeNode[]{this.mothernode});
    ...
    public DefaultMutableTreeNode recursiveWriteNode(String key,
        ...
        //create new path for each component, copy old path passed from last
        // traversal
        DefaultMutableTreeNode[] new_path =
                new DefaultMutableTreeNode[parent_path.length+1];
        System.arraycopy(parent_path, 0, new_path, 0, parent_path.length);
        
        //add current component to new path; add new path to paths for current
        // component
        new_path[parent_path.length] = newnode;
        paths.add(new_path);
    ...
    public Vector< DefaultMutableTreeNode[] > getPathsTo(String id){
        ...
        //Vector< DefaultMutableTreeNode[] > pathsTo =
        // new Vector< DefaultMutableTreeNode[] >();
    ...
    public boolean isSamePathAs(DefaultMutableTreeNode[] path1,
    ...
    public boolean containsComponent(DefaultMutableTreeNode[] path,
        ...
        for (DefaultMutableTreeNode currentNode: path){
            ...
            if ( nodeInfo instanceof Component ) {
                ...
                //if currentCompie is a group node, the path is not a valid
                ...
                //REPLACE isSamsAs with matching id, two compies can be the
                // same but have modified properties
                if (compie.getID().equals(currentCompie.getID())) {
                    return true;
                }
            }
        }   
        //Vector< DefaultMutableTreeNode > vPath =
        // new Vector( java.util.Arrays.asList( path ) );
        return contains;
    }


    public boolean containsNode(DefaultMutableTreeNode[] path,
            DefaultMutableTreeNode node){
        ...
        for (DefaultMutableTreeNode currentNode: path){
            ...
            if (currentNodeInfo instanceof Component){
                ...
                //REPLACE isSamsAs with matching id, two compies can be the
                // same but have modified properties
                if ( nodeCompie.getID().equals(currentCompie.getID()) ) {
                    return true;
                }
            }
        }    
    ...
    public boolean hasGroupNodeAsAncestor( DefaultMutableTreeNode[] pathTo,
            Component compie ){
        //iterate thru each node in the path
        for ( DefaultMutableTreeNode currentNode: pathTo ){
            ...
            if ( nodeInfo instanceof Component ){
                ...
                //if ( !currentCompie.getIsPrimary() &&
                //     !currentCompie.isSameAs(compie) ) {//MAZE:
                //REPLACE isSamsAs with matching id, two compies can be the
                // same but have modified properties
                if ( !currentCompie.getIsPrimary() &&
                     !currentCompie.getID().equals(compie.getID()) ) {
                    return true;
                }
            }
        }
    ...
    public boolean isPathInNamespace( DefaultMutableTreeNode[] path,
            Component rootCompie ){
        ...
        for ( DefaultMutableTreeNode node: path ){
            ...
            if ( nodeInfo instanceof Component ){
                Component currentCompie = ( Component ) nodeInfo;
                //if currentCompie is a group node
                if ( !currentCompie.getNamespace().equals(
                     rootCompie.getNamespace()) ) {
                    return false;
                }
            }
        }
    ...
    public Component getComponent(String id){
        ...
        try{
            ...
            //System.out.println("compie: " + compie.getID());
            return compie;
        }
