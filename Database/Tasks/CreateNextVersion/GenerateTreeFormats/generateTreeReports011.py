...
from hgu.anatomyDb.version011 import Anatomy

import ReportTree011
import ReportFile011
...
perspectiveTree = ReportTree011.ReportTree(config["PERSPECTIVE"],
                                        config["DEPTH_LIMIT"])
...
for format in config["OUTPUT_FORMATS"]:
    for abstractType in config["ABSTRACT_REPORTS"]:
        ...
        ReportFile011.writeAbstractReport(
    ...
    for stageType in config["STAGE_REPORTS"]:
        for togetherness in config["STAGE_FILES"]:
            ...
            ReportFile011.writeStageReport(
