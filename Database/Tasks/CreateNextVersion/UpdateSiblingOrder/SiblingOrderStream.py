...
Code to parse Sibling Order file.

Sibling Order files are produced by the anatomy tree reporting
scripts, using the sibling order that existed in the previous
...
This reads in the files after they have been manually edited, and
generates SQL updates to make the database agree with the ordering
in the sibling order file.
"""
...
# The code that produces these header lines (and everything else in
# the sibling order files) is in the tree reports code.
...
GROUP_HEADER_LINE  = r"^Abstract .+ Showing Only Groups\n"
GROUP_NOTE_LINES   = [
...
class SiblingOrderStream:
    """
    File containing sibling ordering information.  The order is driven by the
    order components are listed in the file.

    This parser reads in a file that show the anatomy in an indented ordered
    form.  The order that items occur in this file is then used by the
    caller to update the order in which items are displayed.
    """
    ...
    def getNextNonBlankLine(self):
        ...
        while (line and
               (line == "\n" or re.match(BOILERPLATE_LINE, line))):
            line = self.__stream.readline()
    ...
    def getNextComponent(self):
        ...
        Each Component line consists of:
          ...
          depth indicator
          component name
          optional synonym list: (syn: syn1, syn2)
        ...
        if re.match(GROUP_HEADER_LINE, line):
            ...
            for expectedLine in GROUP_NOTE_LINES:
                line = self.getNextNonBlankLine()
                if not re.match(expectedLine, line):
                    Util.fatalError([
...
class ComponentLine:
    """
    Contains information available from a component line in the sibling
    order stream.
    """

    def __init__(self, line):
        """
        Parse a component line from a sibling order file into
