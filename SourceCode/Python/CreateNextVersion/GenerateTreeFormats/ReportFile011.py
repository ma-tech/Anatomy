...
from hgu.anatomyDb.version011 import AnaTimedNodeDb
...
class TxtReportFile (LinearReportFile):
    ...
    def addStageAnatomyLine(self, stage, indentLine):
        ...
        #print "timedNode.getDisplayId() = " + timedNode.getDisplayId() 
        indent     = " ".join(indentLine.getIndent())
        if len(indent) > 0:
            indent += " "
...
class XmlReportFile (ReportFile):
    ...
    def addAbstractAnatomyLine(self, indentLine):
        ...
        item.setAttribute("emapaId", node.getDisplayId())
    ...
    def addStageAnatomyLine(self, stage, indentLine):
        ...
        timedComponent.setAttribute("emapId", timedNode.getDisplayId())
        ...
        text = node.getComponentName()
        name.appendChild(self._document.createTextNode(text))
        name.setAttribute("emapId", timedNode.getDisplayId())
        timedComponent.appendChild(name)

        emapaId = self._document.createElement("emapaId")
        text = node.getDisplayId()
        emapaId.appendChild(self._document.createTextNode(text))
        emapaId.setAttribute("emapId", timedNode.getDisplayId())
        timedComponent.appendChild(emapaId)

        groupPathStatus = self._document.createElement("groupPathStatus")
        text = self.GROUP_DISPLAY[anatomyLine.getGroupStatus()]
        groupPathStatus.appendChild(self._document.createTextNode(text))
        groupPathStatus.setAttribute("emapId", timedNode.getDisplayId())
        ...
        for syn in anatomyLine.getSynonyms():
            ...
            text = syn
            synonym.appendChild(self._document.createTextNode(text))
            synonym.setAttribute("emapId", timedNode.getDisplayId())
...
def _writeAbstractGroupsEmbeddedReport(reportTree, reportFile, format):
    """
...
def _writeStageGroupsEmbeddedReport(reportTree, reportFile, stage, format):
    """
...
def writeAbstractReport(reportTree, outputDir, format, reportType):
    ...
    if reportType == GROUPS_EMBEDDED:
        _writeAbstractGroupsEmbeddedReport(reportTree, reportFile, format)
